...
function setAPlayerObserver() {
	try {
		...
		APlayerController.player.on('pause', function (e) {
			...
			var title = document.querySelector('meting-js').aplayer.list.audios[index].title;
			var artist = document.querySelector('meting-js').aplayer.list.audios[index].artist;
			$.message({title: '音乐通知',message: '暂停播放：' + title + ' - ' + artist,type: 'warning'});
		});
...
function updateAPlayerControllerStatus() {
	try {
		if (APlayerController.player.audio.paused) {
			if ( document.getElementsByClassName('nav toggle')[0] ) document.getElementsByClassName('nav toggle')[0].children[0].classList.add("fa-play");
			if ( document.getElementsByClassName('nav toggle')[0] ) document.getElementsByClassName('nav toggle')[0].children[0].classList.remove("fa-pause");
		} else {
			if ( document.getElementsByClassName('nav toggle')[0] ) document.getElementsByClassName('nav toggle')[0].children[0].classList.remove("fa-play");
			if ( document.getElementsByClassName('nav toggle')[0] ) document.getElementsByClassName('nav toggle')[0].children[0].classList.add("fa-pause");
		}
	} catch (error) {
		console.log(error);
	}
...
function updateTitle() {
	...
	try {
		...
		if ( document.getElementsByClassName('nav music-title')[0] ) document.getElementsByClassName('nav music-title')[0].innerHTML = title;
	} catch (error) {
		console.log(error);
	}
}
